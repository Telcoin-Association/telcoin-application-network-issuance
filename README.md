...
Now you can run the main applications.
...
The program accepts the chain(s) to fetch and process information from, the period number being calculated (required), and an optional start block and endblock. The period number is required because the calculator writes its output directly to `rewards/staker_rewards_period_<n>.json` and refuses to overwrite an existing file unless `--rerun` is passed (in which case it writes to `rewards/staker_rewards_period_<n>.rerun.json`).

```bash
# calculate period 29 based on Polygon data only, starting from block 67070000 until block 67078000
yarn start polygon=67070000:67078000 --period=29

# calculate period 29 based on Polygon data, starting from the TANIssuanceHistory's stored `lastSettlementBlock`
yarn start polygon --period=29

# calculate based on Polygon and Mainnet
yarn start polygon mainnet --period=29

# re-run period 29 without clobbering the canonical file (writes to staker_rewards_period_29.rerun.json)
...
## Running the TELxRewardsCalculator

Running the TELx Uniswap v4 rewards calculator for a new period requires the newest period's end block added to `NETWORKS::<chainID>::periodStarts` as well as incrementation of the `PERIODS` array.

Once configured, the program can be run and accepts the Uniswap v4 pool's ID as well as the period being processed:

```bash
yarn ts-node backend/calculators/TELxRewardsCalculator.ts <poolId>:<period>
```

There is also a utility for converting the JSON outputs to XLSX format for non-technical readers to better view:

```bash
yarn ts-node backend/telxHumanReadable.ts
```

## Running the SafeTxArrayBuilder script

Similarly to the TANIP-1 usage outlined below, the TELx rewards output JSON files can be routed through the safeTxArrayBuilder utility by adding the `--telx` flag eg:
